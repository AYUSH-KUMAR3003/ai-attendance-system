...
# --- PAGE CONFIGURATION ---
st.set_page_config(page_title="QUANTUM AI ATTENDANCE SYSTEM", layout="wide", page_icon="logo.png",  
...
# --- NEW VOICE FUNCTION ---
    ...
    # --- NEW NEURAL VOICE FUNCTION (PYTHON NATIVE VERSION) ---
...
def speak_welcome(text):
    ...
    try:
        ...
        if os.path.exists(output_file):
            ...
            st.audio(audio_bytes, format='audio/mp3', start_time=-10, autoplay=True)
            audio_file.close()
...
def mark_attendance_db(name, subject):
    """Inserts attendance record into DB and shows a Toast notification."""
    ...
    if not cursor.fetchone():
        ...
        msg = f"Welcome {name} Have a Nice Day you Marked {status.replace('🔴','').replace('🟢','')}"
        marked = True
    else:
        # Duplicate
        # --- 🔥 THE TOAST IS HERE ---
        st.toast(f"⚠️ Already Marked: {name}", icon="⚠️")
        
        msg = f"{name}, attendance already marked for {subject}."
        marked = False
...
with tab_take_attendance:
    ...
    with col_info:
        ...
        # Placeholder for audio player so it doesn't jump around
        ...
        if img_buffer:
            ...
            for encodeFace, faceLoc in zip(encodes_cur, faces_cur):
                ...
                if matches[matchIndex] and faceDis[matchIndex] < 0.50:
                    ...
                    # Mark DB and get the message to speak
                    marked_fresh, voice_msg = mark_attendance_db(name, selected_subject_cam)
                    
                    if marked_fresh:
                        st.success(f"✅ Identified & Marked: **{name}**")
                        st.balloons()
                    else:
