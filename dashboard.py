...
def load_encodings():
    ...
    st.success(f"✅ AI Ready! Loaded {len(encodeList)} Student data.")
...
def mark_attendance_db(name, subject):
    ...
    if not cursor.fetchone():
        ...
        st.toast(f"⚠️ {name} already marked for {subject}", icon="⚠️")
...
with col1:
    # Ensure logo.png exists in your folder or this line will create an empty space
        ...
        if len(subjects_on_date) > 0:
            ...
            with c1:
                if not df_daily.empty:
                    fig = px.bar(
                        df_daily.sort_values('time_marked'), 
                        x='student_name', 
                        y='time_marked', 
                        color='status', 
            ...
            if total_classes > 0:
                ...
                st.dataframe(
                    att_counts.style.format({"Percentage": "{:.1f}%"}).map(
                        lambda v: 'color: red; font-weight: bold' if 'CRITICAL' in v else 'color: green', 
                        subset=['Status']
                    ),
